...
- Applies $\log_{10}((1000 \times \text{normalized\_value}) + 1)$ transformation (or deviance transformation if specified)
...
Determine the optimal component/rank by plotting the variance explained ($R^2X$) across different ranks for both RISE and PCA. This helps balance model complexity with explanatory power.
...
<figure markdown="span">
  ![Variance Explained](assets/tutorial_images/step2_r2x.png){ width="500" }
</figure>
...
<figure markdown="span">
  ![Factor Match Score](assets/tutorial_images/step3_fms.png){ width="500" }
</figure>
...
Two functions are available in `scrise.rank_selection`:
...
```python
import matplotlib.pyplot as plt
from scrise.plotting import plot_rank_optimization
from scrise.rank_selection import optimize_rank
...
The output of `pf2` includes the original AnnData object with added results and the reconstruction error ($R^2X$). The following are added to the AnnData object:
...
- **Optimized Product Quantization (OPQ)**: Compresses the continuous per-cell projection matrix (`X.obsm["projections"]`) to compact 8-bit quantization codes while guaranteeing high reconstruction fidelity ($R^2 \ge 0.99$).
- **Deterministic Reconstruction**: Factor matrices (`Pf2_A`, `Pf2_B`, `Pf2_weights`, `Pf2_C`) are stored in single precision (`float32`), and weighted projections are deterministically reconstructed on load via $\text{projections} \times \text{Pf2\_B}$, eliminating redundant data on disk.
...
<figure markdown="span">
  ![Condition Factors Heatmap](assets/tutorial_images/step5_condition_factors.png){ width="700" }
  <figcaption><strong>Condition Factors Heatmap.</strong> This heatmap shows how each experimental condition (rows) contributes to each component (columns).</figcaption>
</figure>
...
<figure markdown="span">
  ![Cell Embedding](assets/tutorial_images/step6_cell_embedding.png){ width="700" }
</figure>
...
<figure markdown="span">
  ![Eigen-state Factors Heatmap](assets/tutorial_images/step7_eigenstate_factors.png){ width="400" }
  <figcaption><strong>Eigen-state Factors Heatmap.</strong> This heatmap shows how each eigen-state (representing groups of similar cells) is related to each component. High values indicate strong association with a component.</figcaption>
</figure>
...
<figure markdown="span">
  ![Gene Factors Heatmap](assets/tutorial_images/step8_gene_factors.png){ width="650" }
  <figcaption><strong>Gene Factors Heatmap.</strong> This heatmap shows which genes (rows) are associated with each component (columns). The weight parameter filters out genes with low contributions for easier interpretation.</figcaption>
</figure>
...
<figure markdown="span">
  ![Gene Expression on PaCMAP](assets/tutorial_images/step9_gene_expression.png){ width="700" }
  <figcaption><strong>Gene Expression on PaCMAP.</strong> This visualization overlays MS4A1 gene expression (a B cell marker) onto the PaCMAP embedding. Cells are colored by expression level, revealing which populations express the gene.</figcaption>
</figure>
...
<figure markdown="span">
  ![Weighted Projections for Component 10](assets/tutorial_images/step10_weighted_projections.png){ width="700" }
  <figcaption><strong>Weighted Projections for Component 10.</strong> This plot shows which cells contribute most strongly to component 10. Cells with high weighted projections (bright colors) are most representative of that component's pattern.</figcaption>
</figure>
